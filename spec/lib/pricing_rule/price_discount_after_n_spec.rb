...
RSpec.describe Pricing::PriceDiscountAfterN do
  let(:item) { build(:item, price: 2) }
  let(:discount) do
    described_class.new('Name', item, after_count: 2, amount_of_discount: 1)
  end
  ...
  context 'when the count of item is enough to satisfy the discount' do
    it 'returns a reduced total' do
      ...
      expect(total_after_discount).to eq(total - 3 * 1)
    end
  end
end
